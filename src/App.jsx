    ...
    const handleGenerate = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.id) {
            // Generate a clean internal link format: app.com?v=ID
            const internalLink = `${window.location.origin}${window.location.pathname}?v=${result.id}`;
            setGeneratedUrl(internalLink);
            console.log("[App] Final Link generated with ID:", internalLink);
          } else {
            setGeneratedUrl(result.short_url || result.url);
          }
        } else {
          // Fallback if custom shortener fails or is too long for it too
